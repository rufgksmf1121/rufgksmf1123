...
export function configureTables(){
  document.querySelectorAll('.js-pub').forEach(tbl=>{
    ...
    buildLinks(tbl); applyRowStates(tbl);
...
function buildLinks(tbl){
  ...
  Array.from(tbl.tBodies[0]?.rows||[]).forEach(tr=>{
    ...
    const linkCell = tds[LINK_COL]; const a = linkCell?.querySelector('a');
    if(!a) return; const base = (a.textContent||'').trim(); if(!base) return;
    a.href = computeHref(code, path, base); a.target='_blank'; a.title='새 창에서 열기';
  });
}
export function computeHref(code, path, base){
  const parts = ['page']; if(code){ parts.push(code); if(path) parts.push(path); } else { parts.push(path||''); }
