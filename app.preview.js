import { isPreviewEnabled } from './app.table.js?v=4';
export function installHoverPreview({ selector, width, height, scale, offsetX, offsetY }){
  document.querySelectorAll(selector).forEach(a=>{
    a.addEventListener('mouseenter', e=>{
      ...
      f.src = a.href; pos(f, e.pageX, e.pageY, {width,height,scale,offsetX,offsetY}); f.hidden=false;
    });
    a.addEventListener('mousemove', e=>{ const f=document.getElementById('hoverPreview'); if(!f||f.hidden) return; pos(f, e.pageX, e.pageY, {width,height,scale,offsetX,offsetY}); });
    a.addEventListener('mouseleave', ()=>{ const f=document.getElementById('hoverPreview'); if(f) f.hidden=true; });
  });
}
...
function pos(f, x, y, opt){
  const {width,height,scale,offsetX,offsetY}=opt; const ww=innerWidth, wh=innerHeight;
  const fw=width*scale, fh=height*scale; let left=x+offsetX, top=y+offsetY;
