import { isPreviewEnabled } from './app.table.js?v=10';
const cache = new Map();
...
export function installHoverPreview({ selector, width, height, scale, offsetX, offsetY }){
  document.querySelectorAll(selector).forEach(a=>{
    a.addEventListener('mouseenter', async (e)=>{
      ...
      f.src=href; position(f, e.pageX, e.pageY, {width,height,scale,offsetX,offsetY}); f.hidden=false;
    });
    a.addEventListener('mousemove', (e)=>{
      const f = document.getElementById('hoverPreview'); if (!f || f.hidden) return;
      position(f, e.pageX, e.pageY, {width,height,scale,offsetX,offsetY});
    });
...
function position(f,x,y,opt){ const {width,height,scale,offsetX,offsetY}=opt; const fw=width*scale,fh=height*scale; let left=x+offsetX, top=y+offsetY; if(innerWidth-(x-scrollX)<fw+offsetX*2) left=x-fw-offsetX; if(innerHeight-(y-scrollY)<fh+Math.abs(offsetY)) top=y-fh-Math.abs(offsetY); f.style.left=left+'px'; f.style.top=top+'px'; }
